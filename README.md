...
The images `jamesmcclain/geowave:ef2182d` and `jamesmcclain:geoserver:ef2182d` can be built by typing:
```bash
make
```
...
docker run -it --rm -p 50095:50095 --net=geowave --hostname leader --name leader jamesmcclain/geowave:ef2182d
```

Optional additional followers can be started by typing:
```bash
docker run -it --rm --net=geowave --entrypoint /scripts/follower.sh jamesmcclain/geowave:ef2182d
```

# Use the Command Line Tools #
The GeoWave command line tools can be accessed by starting the container
```bash
docker run -it --rm --net=geowave jamesmcclain/geowave:ef2182d bash
```
...
```bash
docker run -it --rm --net=geowave -p 8080:8080 jamesmcclain/geoserver:ef2182d
```
