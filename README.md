...
The images `jamesmcclain/geowave:e62deaf` and `jamesmcclain:geoserver:e62deaf` can be built by typing:
```bash
make
```
...
docker run -it --rm -p 50095:50095 --net=geowave --hostname leader --name leader jamesmcclain/geowave:e62deaf
```

Optional additional followers can be started by typing:
```bash
docker run -it --rm --net=geowave --entrypoint /scripts/follower.sh jamesmcclain/geowave:e62deaf
```

# Use the Command Line Tools #
The GeoWave command line tools can be accessed by starting the container
```bash
docker run -it --rm --net=geowave jamesmcclain/geowave:e62deaf bash
```
...
```bash
docker run -it --rm --net=geowave -p 8080:8080 jamesmcclain/geoserver:e62deaf
```
